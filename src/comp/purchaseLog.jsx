...
function PurchaseLogTable({ queries, reload }) {
  ...
  const queriesStatusAttribute = ["all", "done", "pending"];
  ...
  useEffect(() => {
    ...
    if (chosedFilter === "all") {
      ...
      UpdatedData = queries.documents.filter(
        (query) => query.payment === chosedFilter
      );
    }

    if (searchText !== "") {
      UpdatedData = UpdatedData.filter((query) =>
        query.order_id.includes(searchText)
      );
    }
  ...
  const TableAttributes = ["Sr No.", ...parameters];
                    ...
                    return (
                      <tr
                        onClick={() => {
                          ...
                          navigate(`/courses/${query[parameters[2]]["id"]}`);
                        }}
                        key={index}
                        className={`border-b w-full dark:border-gray-700 ${(() => {
                          if (query.payment === "done") {
                            return "bg-green-400 text-white";
                          } else if (query.payment === "pending") {
                              ...
                              <div className="dropdown dropdown-hover">
                                ...
                                >
                                    {(()=>{
                                        ...
                                        return stringy(parameters[index + 1]).capitalize()
                                        .s
                                    })()}
