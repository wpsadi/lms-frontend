...
function QueryLogTable({ queries,reload }) {
  ...
  useEffect(()=>{
    ...
    if (chosedFilter === "all"){
        ...
        UpdatedData =  queries.documents.filter((query) => query.status === chosedFilter)
    }

    if (searchText !== ""){
        UpdatedData = UpdatedData.filter((query) => query.$id.includes(searchText))
    }
...
//   console.log("parameters", parameters)
  ...
  return (
    <>
                ...
                <tbody className="w-full">
                    {
                        data.slice(start,end).map((query, index) => {
                            return <tr className={`border-b w-full dark:border-gray-700 ${(()=>{
      ...
      {
          new Array(parameters.length-1).fill("").map((_, index) =>(
          <td className="px-4 py-3 " key={index}>
 <div className="dropdown dropdown-hover">
  <div tabIndex={0} role="button" className="  z-10 "> <p className="line-clamp-2">

{
(()=>{
    if (parameters[index+1] === "$createdAt"){
        return new Date(query[parameters[index+1]]).toLocaleString();
    }
    else if (parameters[index+1] === "status"){
        return (query[parameters[index+1]]).toUpperCase();
    }
    return String(query[parameters[index+1]] ?? "null")
})()
}
    </p></div>
  <div tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 text-black">
    {stringy(parameters[index+1]).capitalize().s }
    </div>
</div>
           </td>
          ))
            ...
            >
              ...
              <ul className="inline-flex items-stretch -space-x-px">
                ...
                <li>
                    ...
                    onClick={()=>{
                        setStart(start+limit)
                        setEnd(end+ (data.length-end >= limit ? limit : data.length%limit))
